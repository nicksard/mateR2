...
mat.stats <- function(mat) {
    ...
    stats_df <- data.frame(
      ...
      # Mating metrics
      mean.male.mates = round(mean(male_mates), 2),
      min.male.mates = min(male_mates),
      max.male.mates = max(male_mates),

      mean.female.mates = round(mean(female_mates), 2),
      min.female.mates = min(female_mates),
      max.female.mates = max(female_mates),

      # The true biological network average (2E / Np)
      overall.mean.mates = round((2 * mp.count) / (num.males + num.females), 2),
