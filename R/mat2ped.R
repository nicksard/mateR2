...
#'   When the matrix carries dimnames these are used as the parent identifiers;
#'   positional `dad1`/`mom1` labels are generated only as a fallback.
#' @return A data frame with three columns: `off` (offspring ID), `mom` (mother ID),
#'   and `dad` (father ID), one row per offspring in the full cohort.
#' @export
mat2ped <- function(mat) {
  ...
  # Offspring IDs are alphanumeric: COLONY allows letters and digits only.
  ped_expanded$off <- paste0("off", 1:nrow(ped_expanded))

  # Matrix dimnames are authoritative when present, so an individual keeps the
  # identity assigned upstream. Positional labels are only a fallback.
  dad_ids <- rownames(mat)
  mom_ids <- colnames(mat)
  if (is.null(dad_ids)) dad_ids <- paste0("dad", seq_len(num_rows))
  if (is.null(mom_ids)) mom_ids <- paste0("mom", seq_len(num_cols))

  ped_expanded$mom <- mom_ids[ped_expanded$mom]
  ped_expanded$dad <- dad_ids[ped_expanded$dad]

  # Reorder columns to the standard pedigree format
  ped_expanded <- ped_expanded[, c("off", "mom", "dad")]
  rownames(ped_expanded) <- NULL
