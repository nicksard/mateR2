...
double cpp_calculate_target_log_prob(
  ...
  // --- BIOLOGICAL MATH FIX: Average of Averages ---
  double male_mean_mates = (Nm > 0) ? (Total_Matings / Nm) : 0.0;
  double female_mean_mates = (Nf > 0) ? (Total_Matings / Nf) : 0.0;
  double Overall_Mean_Mates_actual = (male_mean_mates + female_mean_mates) / 2.0;
...
List run_mcmc_sampler_cpp(
  ...
  for (int t = 0; t < n_iter; ++t) {
    ...
    // --- BIOLOGICAL MATH FIX: Average of Averages (History Output) ---
    double current_male_mm = (current_Nm > 0) ? (current_Total_Matings / current_Nm) : 0.0;
    double current_female_mm = (current_Nf > 0) ? (current_Total_Matings / current_Nf) : 0.0;
    double current_mean_mates = (current_male_mm + current_female_mm) / 2.0;
